...
pub extern fn inb(port: c_int) u8;
pub extern fn inw(port: c_int) u16;
pub extern fn inl(port: c_int) u32;
pub extern fn outb(value: u32, port: c_int) void;//{
//     _=asm ("outb %%al, %%dx"
//         : [dx] "{dx}" (value)
//         : [al] "{al}" (port)
//     );
// }
pub extern fn outw(value: u32, port: c_int) void;
pub extern fn outl(value: u32, port: c_int) void;
pub fn iowait() void {
    outb(0,0x80);
}
pub fn inb_slow(port: c_int) u8 {
    var result: u8 = inb(port);
    iowait();
    return result;
}
pub fn inw_slow(port: c_int) u16 {
    var result: u16 = inw(port);
    iowait();
    return result;
}
pub fn inl_slow(port: c_int) u32 {
    var result: u32 = u32(inl(port));
    iowait();
    return result;
}
pub fn outb_slow(value: u8, port: c_int) void {
    outb(value, port);
    iowait();
}
pub fn outw_slow(value: u16, port: c_int) void {
    outw(value, port);
    iowait();
}
pub fn outl_slow(value: u32, port: c_int) void {
    outl(value, port);
    iowait();
}
